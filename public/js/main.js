(() => {
  ...
  $('btn-create-confirm').addEventListener('click', () => {
    ...
    Network.createRoom({ name: Settings.current.name, maxPlayers, mode: 'tetris' }, (res) => {
  ...
  $('btn-ready-toggle').addEventListener('click', () => {
    myReady = !myReady;
    $('btn-ready-toggle').textContent = myReady ? '準備をキャンセル' : '準備OK！';
  ...
  $('btn-start-game').addEventListener('click', () => {
    ...
    $('btn-start-game').textContent = '開始しています…';
    Network.startGame();
  });
  ...
  function renderLobby(state) {
    ...
    state.players.forEach((p, idx) => {
      ...
      if (mine) { myMode = p.mode; myReady = p.ready; applyMyModeUI(p.mode); $('btn-ready-toggle').textContent = p.ready ? '準備をキャンセル' : '準備OK！'; }
    });
    ...
    $('lobby-spectate-note').classList.toggle('hidden', !isSpectator);
    ...
    $('btn-start-game').textContent = '対戦開始（全員準備OKで押せます）';
  ...
  function handleAction(action) {
    ...
    if (action === 'targetPrev' || action === 'targetNext') {
      Network.cycleTarget(action === 'targetPrev' ? 'prev' : 'next');
      return;
    }
  ...
  function renderMyView(payload) {
    ...
    others.forEach((p) => {
      ...
      name.textContent = (me && me.manualTarget === p.id ? '🎯 ' : '') + p.name + (p.isCPU ? '(CPU)' : '');
