...
io.on('connection', (socket) => {
  ...
  socket.on('leaveRoom', () => {
    ...
    if (result) {
      ...
      if (!result.deleted) {
        ...
        if (result.room.status === 'playing' || result.room.status === 'results') {
          io.to(result.roomId).emit('gameState', result.room.gameStateFor());
        }
      } else {
        io.to(result.roomId).emit('roomClosed');
      }
    }
  });
  ...
  socket.on('cycleTarget', (direction) => {
    ...
    const room = manager.get(roomId);
    if (!room) return;
    room.cycleTarget(socket.id, direction === 'prev' ? 'prev' : 'next');
  });
